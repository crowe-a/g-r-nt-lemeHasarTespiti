...
def conatact(healthy):
    return print(healthy)



# CartCurt.armOlVeYuksel(10)
class SolarPanel:
    ...
    def load_model(self, model_name):
        ...
        """
        if model_name:
            model = torch.hub.load('C:/Users/darkr/Desktop/Otonom/yolov5-master/yolov5-master', 'custom', path=model_name, source="local")
        # else:
    ...
    def plot_boxes(self, results, frame):
        ...
        for i in range(n):
            ...
            if row[4] >= 0.3:
                ...
                valu=self.class_to_label(labels[i])#get input for take photo and location
                cv2.putText(frame, self.class_to_label(labels[i]), (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 0.9, bgr, 2)
                #valu=self.class_to_label(labels(i))
                print(valu)
                if "BakimGereken"==valu:
                    healthy=1
                    conatact(healthy)
                    # if feedback():
...
detector = SolarPanel(capture_index=0, model_name='gunespaneli0.pt')
detector()